...
int main(int argc, char *argv[]){
	...
	// Keep connector output focused on important events only.
	// Default to ERROR logs; enable INFO logs when debug is requested.
	mdclog_severity_t log_level = MDCLOG_ERR;
	const char* xapp_log_level = std::getenv("XAPP_LOG_LEVEL");
	if (xapp_log_level != NULL && std::string(xapp_log_level) == "DEBUG") {
		log_level = MDCLOG_INFO;
	}

	mdclog_attr_t *attr;
	mdclog_attr_init(&attr);
	mdclog_attr_set_ident(attr, "hw_xapp_main");
	mdclog_init(attr);
