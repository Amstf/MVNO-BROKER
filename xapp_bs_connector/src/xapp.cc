...
static bool is_debug_enabled_xapp() {
    const char* dbg = std::getenv("XAPP_VERBOSE_DEBUG");
    return dbg != NULL && std::string(dbg) == "1";
}

Xapp::Xapp(XappSettings &config, XappRmr &rmr, bool protobuf){

    if (protobuf && is_debug_enabled_xapp()) {
...
void Xapp::startup_subscribe_requests(void ){
    ...
    // ensure unique ricInstanceId per gNB to avoid clashes inside the SubMgr
    unsigned int ric_instance_id = 1;

    auto gnblist_raw = get_rnib_gnblist();
    std::vector<std::string> gnblist;
    std::unordered_set<std::string> seen;

    for (const auto &id : gnblist_raw) {
        if (seen.insert(id).second) {
            gnblist.push_back(id);
        } else {
            mdclog_write(MDCLOG_INFO, "Skipping duplicate gNB %s from RNIB list", id.c_str());
        }
    }
    ...
    for(int i = 0; i<sz; i++){
        ...
        // SubMgr only accepts one outstanding subscription procedure at a time.
        // Wait until any previous request finishes before issuing the next gNB.
        while (subhandler_ref != NULL && subhandler_ref->has_pending_requests()) {
            mdclog_write(MDCLOG_INFO, "Pending subscription detected, waiting before sending request for %s", gnblist[i].c_str());
            std::this_thread::sleep_for(std::chrono::milliseconds(200));
        }

        if (subhandler_ref != NULL && subhandler_ref->is_request_entry(gnblist[i])) {
            mdclog_write(MDCLOG_INFO,"Skipping subscription for %s because a request is already pending", gnblist[i].c_str());
            continue;
        }
        ...
        E2APSubscriptionRequest<HWEventTriggerDefinition, HWActionDefinition> requestObj(
            E2APSubscriptionRequest<HWEventTriggerDefinition, HWActionDefinition>::SubscriptionRequestIEs()
                .set_ranFunctionID(1)
                .set_ricInstanceID(ric_instance_id)
                .set_ricRequestorID(3)
                .set_ricAction_ToBeSetup_List(actionObj)
                .set_ricEventTriggerDefinition(eventObj));
        ...
        // increment ricInstanceId for the next target gNB
        ric_instance_id++;
    }
}
...
void Xapp::set_rnib_gnblist(void) {
    ...
    rnib_gnblist.clear();
    std::unordered_set<std::string> seen_gnbs;
    ...
    for (SizeType i = 0; i < gnblist.Size(); i++) // Uses SizeType instead of size_t
    {
        ...
        if (seen_gnbs.insert(name).second) {
            rnib_gnblist.push_back(name);
        } else {
            mdclog_write(MDCLOG_INFO, "Skipping duplicate gNB %s from RNIB list", name.c_str());
        }
...
void Xapp::handle_rx_msg_agent(std::string agent_ip) {
    ...
    char buf[max_size] = {0};
    auto recv_all = [](int fd, char *out, size_t len) -> bool {
        size_t total = 0;
        while (total < len) {
            ssize_t received = recv(fd, out + total, len - total, 0);
            if (received == 0) {
                return false;
            }
            if (received < 0) {
    ...
    while (true) {
        if (agent_socket_fd > -1) {
            if (!using_protobuf) {
                ...
                std::string gnb_id = buf_json.value("gnb_id", "");
                std::string control = buf_json.value("control", "");
            ...
            unsigned char meid_len = 0;
            if (!recv_all(agent_socket_fd, reinterpret_cast<char*>(&meid_len), 1)) {
                std::cout << "Agent connection closed while reading MEID length" << std::endl;
                break;
            }
            if (meid_len == 0 || meid_len > max_size - 3) {
                std::cout << "WARNING: invalid MEID header length " << static_cast<int>(meid_len) << std::endl;
                continue;
            }

            if (!recv_all(agent_socket_fd, buf, meid_len)) {
                std::cout << "Agent connection closed while reading MEID bytes" << std::endl;
                break;
            }
            std::string gnb_meid(buf, buf + meid_len);

            unsigned char len_bytes[2] = {0, 0};
            if (!recv_all(agent_socket_fd, reinterpret_cast<char*>(len_bytes), 2)) {
                std::cout << "Agent connection closed while reading payload length" << std::endl;
                break;
            }
            size_t payload_len = (static_cast<size_t>(len_bytes[0]) << 8) | static_cast<size_t>(len_bytes[1]);
            if (payload_len == 0 || payload_len > max_size) {
                std::cout << "WARNING: invalid payload length " << payload_len << std::endl;
                continue;
            }
            if (!recv_all(agent_socket_fd, buf, payload_len)) {
                std::cout << "Agent connection closed while reading payload bytes" << std::endl;
                break;
            }

            std::cout << "Sending RIC control request to gNB with ID " << gnb_meid << std::endl;
            send_ric_control_request(buf, static_cast<int>(payload_len), gnb_meid);
