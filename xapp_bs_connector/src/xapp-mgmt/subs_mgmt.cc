...
void SubscriptionHandler::manage_subscription_response(int message_type, const transaction_identifier &id){
  std::this_thread::sleep_for(std::chrono::milliseconds(1000));

  std::unique_lock<std::mutex> _local_lock(*(_data_lock.get()));
  mdclog_write(MDCLOG_INFO,
               "Subscription Handler: Status for me id %s WAS: %d",
               id.c_str(), this->get_request_status(id));
  ...
  mdclog_write(MDCLOG_INFO,
               "Subscription Handler: Status for me id %s IS: %d",
               id.c_str(), this->get_request_status(id));
}
...
bool SubscriptionHandler::has_pending_requests(){
  std::lock_guard<std::mutex> lock(*(_data_lock).get());

  for (const auto &entry : status_table) {
    if (entry.second == request_pending) {
      return true;
    }
  }
